...
import { FaHome, FaTasks, FaUserAlt, FaShoppingCart, FaNewspaper, FaBox, FaQuestionCircle, FaRecycle, FaTimes } from "react-icons/fa";
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-auto my-8 overflow-hidden" onClick={(e) => e.stopPropagation()}>
        ...
        {cartItems.length > 0 && (
          <div className="p-5 bg-gray-50 border-t">
            ...
            <button onClick={onCheckout} className="w-full py-3 bg-green-600 text-white font-semibold rounded-lg hover:bg-green-700 transition-colors disabled:bg-gray-400">
              Proceed to Checkout
            </button>
          </div>
        )}
...
const HelloUser = () => {
  ...
  const ProductCard = React.memo(({ product }) => {
    ...
    const handleAdd = () => {
      // **BUG FIX**: Calculate total as a number, not a string.
      ...
      const newEntry = {
        ...
        quantity,
        total: numericTotal, // Store as a number
        ...
        <div className="flex-grow">
          ...
          <div className="flex items-center justify-between mt-2">
            <div className="flex items-center border border-gray-200 rounded-md">
              <button onClick={() => setQuantity(q => Math.max(1, q - 1))} className="px-2 py-1 text-lg text-gray-600 hover:bg-gray-100 rounded-l-md focus:outline-none">-</button>
              ...
              <button onClick={() => setQuantity(q => q + 1)} className="px-2 py-1 text-lg text-gray-600 hover:bg-gray-100 rounded-r-md focus:outline-none">+</button>
            </div>
