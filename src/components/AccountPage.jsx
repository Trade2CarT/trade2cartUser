              ...
              <div className="mt-3 text-sm text-gray-600 space-y-3">
                {userLoading ? <p>Loading profile...</p> : editableUserData ? (
                  <div className="space-y-4">
                    ...
                    <div className="flex justify-end gap-3">{isEditing ? (<><button onClick={() => { setIsEditing(false); setEditableUserData(userData); }} className="px-4 py-2 bg-gray-300 rounded-lg">Cancel</button><button onClick={handleProfileUpdate} className="px-4 py-2 bg-green-500 text-white rounded-lg flex items-center gap-2"><FaSave /> Save</button></>) : (<button onClick={() => setIsEditing(true)} className="px-4 py-2 bg-blue-500 text-white rounded-lg flex items-center gap-2"><FaEdit /> Edit</button>)}</div>
                  </div>
